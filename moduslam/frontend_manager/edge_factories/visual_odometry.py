...
class VisualOdometryEdgeFactory(EdgeFactory):
    ...
    @property
    def vertices_types(self) -> set[type[CameraPose]]:
        """Types of the used vertices.

        Returns:
            set with 1 type (CameraPose).
        """
        return {CameraPose}
    ...
    def create(
        ...
        Args:
            ...
            timestamp: timestamp of the j-th camera pose.
    ...
    def _create_edge(
        self, vertex1: CameraPose, vertex2: CameraPose, measurement: Measurement
    ) -> VisualOdometry:
        """Creates an edge.
    ...
    def _get_vertices(
        ...
        Returns:
            2 vertices.
        """
        v1 = find_vertex(CameraPose, storage, t1, time_margin)
        if not v1:
            p = find_vertex(Pose, storage, t1, time_margin)
            if p:
                v1 = CameraPose(timestamp=p.timestamp, index=p.index, value=p.value)

        v2 = find_vertex(CameraPose, storage, t2, time_margin)
        if not v2:
            p = find_vertex(Pose, storage, t2, time_margin)
            if p:
                v2 = CameraPose(timestamp=p.timestamp, index=p.index, value=p.value)
        ...
        if v1 and not v2:
            v2 = CameraPose(timestamp=t2, index=new_index, value=v1.value)
            return v1, v2

        elif v2 and not v1:
            v1 = CameraPose(timestamp=t1, index=new_index, value=v2.value)
            return v1, v2

        else:
            v1 = CameraPose(timestamp=t1, index=new_index)
            v2 = CameraPose(timestamp=t2, index=new_index + 1)
            return v1, v2
