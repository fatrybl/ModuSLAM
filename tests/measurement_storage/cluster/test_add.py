...
def test_add_continuous():
    m = ContinuousMeasurement([PseudoMeasurement(0)])
    cluster = MeasurementCluster()

    cluster.add(m)

    assert m in cluster
    assert m in cluster.continuous_measurements
    assert cluster.continuous_measurements == (m,)
    assert cluster.measurements == (m,)
