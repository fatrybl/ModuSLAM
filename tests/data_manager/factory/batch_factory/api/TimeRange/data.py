...
binary_data = [(z_sick_back_1[1:],
               ...
               (z_sick_middle_3[1:],
                (DatasetStructure.lidar_2D_middle_dir / str(z_sick_middle_3[0])).with_suffix(DatasetStructure.binary_file_extension)),
               (z_velodyne_left_1[1:],
...
el23 = Element(
    timestamp=z_imu_3[0],
    measurement=Measurement(
        sensor=Encoder(imu.name, params),
        values=tuple(str(i) for i in z_imu_3[1:])),
    location=CsvDataLocation(
        file=imu.file_path,
        position=2))
...
imu_requests: set[PeriodicData] = {
    PeriodicData(sensor=el3.measurement.sensor,
                 period=TimeRange(start=el3.timestamp,
                                  stop=el3.timestamp)),
    PeriodicData(sensor=el10.measurement.sensor,
                 period=TimeRange(start=el10.timestamp,
                                  stop=el10.timestamp)),
    PeriodicData(sensor=el23.measurement.sensor,
...
lidar2D_requests: set[PeriodicData] = {
    PeriodicData(sensor=el5.measurement.sensor,
                 period=TimeRange(start=el5.timestamp,
                                  stop=el5.timestamp)),
...
stereo_requests: set[PeriodicData] = {
    PeriodicData(sensor=el19.measurement.sensor,
                 period=TimeRange(start=el19.timestamp,
                                  stop=el19.timestamp)),
    PeriodicData(sensor=el22.measurement.sensor,
                 period=TimeRange(start=el22.timestamp,
                                  stop=el22.timestamp)),
    PeriodicData(sensor=el24.measurement.sensor,
                 period=TimeRange(start=el24.timestamp,
                                  stop=el24.timestamp)),
    PeriodicData(sensor=el19.measurement.sensor,
                 period=TimeRange(start=el19.timestamp,
                                  stop=el24.timestamp)),
...
imu_scenario: tuple[set[PeriodicData], DataBatch] = (
    imu_requests, imu_batch)
lidar2D_scenario: tuple[set[PeriodicData], DataBatch] = (
    lidar2D_requests, lidar2D_batch)
stereo_scenario: tuple[set[PeriodicData], DataBatch] = (
    stereo_requests, stereo_batch)
...
scenarios: list[tuple[TimeRange, DataBatch]] = [(TimeRange(el1.timestamp, el1.timestamp),
                                                 b1),
