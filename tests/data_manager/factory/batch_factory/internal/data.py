...
binary_data = [
    ...
    (
        z_sick_back_3[1:],
        (DatasetStructure.lidar_2D_back_dir / str(z_sick_back_3[0])).with_suffix(
            DatasetStructure.binary_file_extension
        ),
    ),
    (
        z_sick_back_4[1:],
        (DatasetStructure.lidar_2D_back_dir / str(z_sick_back_4[0])).with_suffix(
            DatasetStructure.binary_file_extension
        ),
    ),
    (
...
image_data = [
    ...
    (
        z_stereo_left_4,
        (DatasetStructure.stereo_left_data_dir / str(z_stereo_left_4[0])).with_suffix(
            DatasetStructure.image_file_extension
        ),
    ),
    (
        z_stereo_right_4,
        (DatasetStructure.stereo_right_data_dir / str(z_stereo_right_4[0])).with_suffix(
            DatasetStructure.image_file_extension
        ),
    ),
]
...
el14 = Element(
    ...
    location=CsvDataLocation(file=encoder.file_path, position=3),
)
...
el16 = Element(
    ...
    measurement=Measurement(sensor=Lidar2D(sick_back.name, params), values=z_sick_back_3[1:]),
    location=BinaryDataLocation(
        file=(DatasetStructure.lidar_2D_back_dir / str(z_sick_back_3[0])).with_suffix(
            DatasetStructure.binary_file_extension
        )
    ),
)
...
el17 = Element(
    ...
    measurement=Measurement(sensor=Lidar2D(sick_back.name, params), values=z_sick_back_4[1:]),
    location=BinaryDataLocation(
        file=(DatasetStructure.lidar_2D_back_dir / str(z_sick_back_4[0])).with_suffix(
            DatasetStructure.binary_file_extension
        )
    ),
)
...
el20 = Element(
    ...
    measurement=Measurement(
        sensor=StereoCamera(stereo.name, params),
        values=(
            Image.fromarray(z_stereo_left_4[1]),
...
encoder_requests: list[PeriodicData] = [
    ...
    PeriodicData(
        sensor=el12.measurement.sensor,
        period=TimeRange(start=el12.timestamp, stop=el12.timestamp),
    ),
    PeriodicData(
        sensor=el1.measurement.sensor,
        period=TimeRange(start=el1.timestamp, stop=el14.timestamp),
    ),
    PeriodicData(
        sensor=el12.measurement.sensor,
        period=TimeRange(start=el12.timestamp, stop=el13.timestamp),
    ),
    PeriodicData(
        sensor=el1.measurement.sensor,
        period=TimeRange(start=el1.timestamp, stop=el12.timestamp),
    ),
    PeriodicData(
        sensor=el13.measurement.sensor,
        period=TimeRange(start=el13.timestamp, stop=el14.timestamp),
    ),
    PeriodicData(
        sensor=el1.measurement.sensor,
        period=TimeRange(start=el1.timestamp, stop=el13.timestamp),
    ),
    PeriodicData(
        sensor=el12.measurement.sensor,
        period=TimeRange(start=el12.timestamp, stop=el14.timestamp),
    ),
]
...
encoder_batches: list[DataBatch] = [
    batch1,
    batch2,
    batch3,
    batch4,
...
encoder_scenarios: list[tuple[PeriodicData, DataBatch]] = list(zip(encoder_requests, encoder_batches))


lidar2D_requests: list[PeriodicData] = [
    PeriodicData(
        sensor=el2.measurement.sensor,
        period=TimeRange(start=el2.timestamp, stop=el2.timestamp),
    ),
    PeriodicData(
        sensor=el17.measurement.sensor,
        period=TimeRange(start=el17.timestamp, stop=el17.timestamp),
    ),
    PeriodicData(
        sensor=el15.measurement.sensor,
        period=TimeRange(start=el15.timestamp, stop=el15.timestamp),
    ),
    PeriodicData(
        sensor=el2.measurement.sensor,
        period=TimeRange(start=el2.timestamp, stop=el17.timestamp),
    ),
    PeriodicData(
        sensor=el15.measurement.sensor,
        period=TimeRange(start=el15.timestamp, stop=el16.timestamp),
    ),
    PeriodicData(
        sensor=el2.measurement.sensor,
        period=TimeRange(start=el2.timestamp, stop=el15.timestamp),
    ),
    PeriodicData(
        sensor=el16.measurement.sensor,
        period=TimeRange(start=el16.timestamp, stop=el17.timestamp),
    ),
    PeriodicData(
        sensor=el2.measurement.sensor,
        period=TimeRange(start=el2.timestamp, stop=el16.timestamp),
    ),
    PeriodicData(
        sensor=el15.measurement.sensor,
        period=TimeRange(start=el15.timestamp, stop=el17.timestamp),
    ),
]
...
lidar2D_batches: list[DataBatch] = [
    batch1,
    batch2,
    batch3,
    batch4,
...
lidar2D_scenarios: list[tuple[PeriodicData, DataBatch]] = list(zip(lidar2D_requests, lidar2D_batches))


stereo_requests: list[PeriodicData] = [
    PeriodicData(
        sensor=el11.measurement.sensor,
        period=TimeRange(start=el11.timestamp, stop=el11.timestamp),
    ),
    PeriodicData(
        sensor=el20.measurement.sensor,
        period=TimeRange(start=el20.timestamp, stop=el20.timestamp),
    ),
    PeriodicData(
    ...
    PeriodicData(
        sensor=el11.measurement.sensor,
        period=TimeRange(start=el11.timestamp, stop=el20.timestamp),
    ),
    PeriodicData(
        sensor=el18.measurement.sensor,
        period=TimeRange(start=el18.timestamp, stop=el19.timestamp),
    ),
    PeriodicData(
        sensor=el11.measurement.sensor,
        period=TimeRange(start=el11.timestamp, stop=el18.timestamp),
    ),
    PeriodicData(
        sensor=el19.measurement.sensor,
        period=TimeRange(start=el19.timestamp, stop=el20.timestamp),
    ),
    PeriodicData(
        sensor=el11.measurement.sensor,
        period=TimeRange(start=el11.timestamp, stop=el19.timestamp),
    ),
    PeriodicData(
        sensor=el18.measurement.sensor,
        period=TimeRange(start=el18.timestamp, stop=el20.timestamp),
    ),
]
...
stereo_scenarios: list[tuple[PeriodicData, DataBatch]] = list(zip(stereo_requests, stereo_batches))
